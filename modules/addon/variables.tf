...
variable "max_history" {
  description = "Maximum number of release versions stored per release. Defaults to `3`. Normally default is 0 (no limit)"
  type        = number
  default     = 3
}
...
variable "set_list" {
  description = "Value block with custom values to be merged with the values yaml that are lists"
  type        = any
  default     = []
}
