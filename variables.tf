...
variable "eks" {
  ...
  Cluster settings:
    ...
    - iam_role_additional_policies: Map of additional IAM policy ARNs to attach to the cluster IAM role, keyed by an arbitrary name (default: {}). When enable_auto_mode is true, must include AmazonEKSBlockStoragePolicyV2 for Auto Mode's block storage capability to work — the module does not attach it automatically.
